...
def exportar_a_excel(tabla, ruta, parametros=None):
    ...
    filas = []
    for fila in tabla:
        for ronda, (rnd1, p1, rnd2, p2, pts, acumulado) in enumerate(fila['detalle'], start=1):
            filas.append({
                ...
                'Supera Objetivo (acumulado)': fila['supera_objetivo']
            })
    ...
    if iteraciones_totales > 0:
        objetivo_superado = tabla[-1]['supera_objetivo']
